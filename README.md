  ...
  .castflow/
    ...
    core/
      ...
      hooks/                            #   trace 采集脚本源码
        ...
        test_evolution.py               #     基础单元测试（84 tests）
        test_100day_simulation.py       #     100 天模拟测试（27 tests）
        test_365day_simulation.py       #     365 天模拟测试（23 tests）
      skills/                           #   核心 Skill
    scripts/                            #   工具脚本
    templates/                          #   生成模板
  README.md
...
三套测试覆盖自我进化系统的完整链路，零外部依赖（仅 Python 标准库 `unittest`）。每次运行都在临时目录中创建隔离环境，不影响项目数据。

```bash
cd CastFlow/.castflow/core/hooks

...
按"行为时点"分层加载，避免每次 Skill 调用都全量加载所有文档。命名约定 `T<序号>-<动词>`。**时点的唯一权威源是项目根 `CLAUDE.md`「使用Skill的分层加载」段**（CLAUDE.md 是 always-applied，自动入上下文，不存在"何时读元约定"的问题）。

**自动入上下文（前提，非时点）**：项目 `CLAUDE.md` + 被触发的 Skill 的 `SKILL.md` 由宿主自动注入。

| 时点 | 触发行为 | AI 主动 Read |
|------|------|---------|
| **T1-PREPARE** | 准备生成或修改代码（写代码前） | GLOBAL_SKILL_MEMORY 协议 1/2 + 该 Skill 的 SKILL_MEMORY.md（如存在）+ 按需 EXAMPLES 章节 |
| **T2-EXECUTE** | 代码生成中决策 IDP 写入 | GLOBAL_SKILL_MEMORY 协议 3 + 按需 protocols/idp-protocol.md |
...
**Skill 内部四文件默认映射**（隐式约定，新 Skill 无需声明）：

| 文件 | 加载方式 | 对应时点 |
|------|---------|---------|
| SKILL.md | 宿主自动注入 | 前提 |
| EXAMPLES.md | AI 按需 Read（章节级） | T1-PREPARE |
| SKILL_MEMORY.md | AI 全文 Read | T1-PREPARE |
| ITERATION_GUIDE.md | AI 全文 Read | T4-MAINTAIN |

**协议文件**（按需加载，不强制每次都读）：

| 文件 | 路径 | 加载时点 |
|------|------|---------|
| idp-protocol.md | `.claude/skills/protocols/` | T2-EXECUTE 且本次会写入 IDP 时 |
| validated-protocol.md | `.claude/skills/protocols/` | T3-FEEDBACK |
| pipeline_protocol.md | `.claude/skills/code-pipeline-skill/config/` | 仅 code-pipeline 执行期间 |

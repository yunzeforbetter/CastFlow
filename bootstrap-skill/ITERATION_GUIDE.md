...
触发条件：生成的文件质量不满足需求
优先级：Medium
文件：.castflow/core/templates/ 与 .castflow/bootstrap-assets/skill-templates/ 下的模板文件
检查清单：
  - [ ] 模板占位符是否完整
...
约束：
  ...
  - Phase 5：子代理以 **skill-creator** 为主路径；手话含任务 + 语言 + 实值占位符 + 按 `SKILL_ITERATION` 必读栈读 **harness** `.castflow/...`；**不**枚举四文件名（制品以已读 `SKILL_ITERATION` 为准）
检查清单：

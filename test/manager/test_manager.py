...
class TestSeedSync(TmpProject):
    def test_seed_writes_runtime_and_rules(self):
        ...
        self.assertTrue(os.path.isdir(os.path.join(self.root, ".claude", "skills", "bootstrap-skill")))
        self.assertTrue(os.path.isdir(os.path.join(self.root, ".agents", "skills", "bootstrap-skill")))
        self.assertTrue(os.path.isfile(os.path.join(
            self.root, ".claude", "skills", "bootstrap-skill", "SKILL.md")))
        self.assertTrue(os.path.isfile(os.path.join(
            self.root, ".agents", "skills", "bootstrap-skill", "SKILL.md")))
    ...
    def test_claude_settings_has_hooks_when_on(self):
        ...
        self.assertIn(".castflow/core/hooks/trace-collector.py", blob.replace("\\\\", "/"))
...
class TestSkillRecallDescriptions(unittest.TestCase):
    ...
    def test_live_skills(self):
        self._assert_recall("bootstrap-skill", "bootstrap-skill", "SKILL.md")
        self._assert_recall(
            "origin-evolve-skill",
    ...
    def test_templates(self):
        self._assert_recall(
            "architect-skill",
            ".castflow", "bootstrap-assets", "skill-templates",
            "architect.template", "SKILL.template.md")
        self._assert_recall(
            "debug-skill",
            ".castflow", "bootstrap-assets", "skill-templates",
            "debug.template", "SKILL.template.md")
        self._assert_recall(
            "profiler-skill",
            ".castflow", "bootstrap-assets", "skill-templates",
            "profiler.template", "SKILL.template.md")
...
class TestSkillIterationMetaSpec(unittest.TestCase):
    ...
    def test_required_contracts_present(self):
        ...
        self.assertIn(".castflow-runtime/skills/", text)
        self.assertIn("python .castflow/manager.py validate", text)

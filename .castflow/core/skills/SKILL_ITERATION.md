...
description: T4-MAINTAIN meta-spec four files SKILL EXAMPLES SKILL_MEMORY ITERATION_GUIDE create skill edit skill skill structure skill format AUTHORING_GUIDE domain README template.md skill review delivery QA NOT daily skill invoke coding T1 T2 code_pipeline
---
...
| **`.claude/templates/AUTHORING_GUIDE.md`** | 写作元规范、与模板配合方式 | 创建/迭代 Skill 正文时**必须**与本文档同时阅读 |
...
| 我要做什么 | 查看 |
|----------|------|
| 创建新 Skill | [必读资料栈](#必读资料栈创建与冷启动统一) + [Skill 文件结构](#skill文件结构标准) + [创建前准备](#创建或迭代前的准备) |
...
- [ ] 已按下文 [必读资料栈](#必读资料栈创建与冷启动统一) 完成与本次创建方式对应的阅读（无现成模板时至少：本文件 + `AUTHORING_GUIDE.md`）
...
**有 CastFlow 脚手架模板时**（常见：architect / debug / profiler）：域 README 与 `*.template.md` 在 **`<harness>/bootstrap-assets/skill-templates/<域>.template/`**（`<harness>` 为项目内包含 `bootstrap.py` 的 `.castflow` 目录，如 `.../CastFlow/.castflow`）。**模块 skill** 在装架后用 **`.claude/templates/skills/programmer.template/`**。

**无现成模板的自由 Skill**：仍**必须**阅读 **本文件** + `AUTHORING_GUIDE.md`；无域 README 则该项跳过，**不得**跳过本文件与 AUTHORING。

**编排层（如 bootstrap 手话、主 agent 派子任务）**：只需说明目标 skill 名、语言、扫描参数等；**不要**在编排文档里再抄一遍上表——**以本段为唯一权威**。是否列举四个文件名以本文件结构标准为准，**不**在编排层当硬性清单重复。

**与 skill-creator**：`skill-creator` 是创作/评测流程工具；**上述必读在流程中仍须落实**，skill-creator 不替代本栈。

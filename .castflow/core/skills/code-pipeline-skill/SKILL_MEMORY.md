...
### 规则 1：PIPELINE_CONTEXT.md 单一事实来源 + PCB 常驻

所有工程信息通过 PIPELINE_CONTEXT.md 流转。结构：

- **头部 PCB 区**（常驻，5 子标题必存在）：`SHADOW_BANS` / `CONFIG_SYNTHESIS` / `MACRO_SCOPE` / `BLUEPRINT` / `ATOMIC_EXECUTION`
- **尾部 Step 段落**（追加）：Step 1-9
...
### 规则 2：Step 1 - 需求拆分 + API 声明 + run_id 生成
...
- [ ] 输入含 PDF / 导图 / 截图 -> 先执行 pipeline_protocol 协议 2 双阶段解构
...
何时执行：跨 3+ 模块、新约束、高风险集成。何时跳过：单模块、API 全部已存在。

按 pipeline_protocol 协议 1/4 填充 PCB：

- **L1 动态**（`config/params.schema.json` + `defaults.json`）选定执行步骤
- **L2 静态**（CLAUDE.md + GLOBAL_SKILL_MEMORY + 相关 SKILL_MEMORY）提取硬性红线
- 合成映射（每项标注来源）：
  - 命名规范 / 命名空间 / 基类 -> `PCB.CONFIG_SYNTHESIS`
  - "禁止 X"类规则 -> `PCB.SHADOW_BANS`
  - 功能点 / 跳转动线 -> `PCB.MACRO_SCOPE`
  - 类签名 / 事件契约 -> `PCB.BLUEPRINT`
  - 原子任务拆分 `[ ]` -> `PCB.ATOMIC_EXECUTION`
...
3. 给出判定：
   - **GO**：无 BLOCKER，一致或轻微差异 -> validated=true
   - **GO-WITH-CAUTION**：有 CompletableTODO，进入 Step 6 -> validated=true
   - **NO-GO**：有 BLOCKER 需返工（返回 Step 1 或 3） -> validated=false
4. 写 `.claude/traces/.pending_pipeline_result.json`（JSON 格式，字段 `pipeline_run_id` + `result`），由 trace-flush hook 批量回填
...
- **Cleanup 模式**：删除 PIPELINE_CONTEXT.md / PIPELINE_INDEX.md / `temp/pipeline-output/`（run_id 随文件消失）
- **Persist 模式**：全部保留，但**必须从 PIPELINE_CONTEXT.md 手动删除 `pipeline_run_id:` 行**，防止下次 pipeline 的新 trace 被误关联

**检查**：Persist 模式结束前确认 run_id 行已移除。中途放弃的 pipeline 应告知用户 7 天后 trace-flush 会将 `pending-pipeline` 条目标记为 invalid。

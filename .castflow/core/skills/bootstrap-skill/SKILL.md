...
将分析结果写入 bootstrap-output/content/architect/ 目录：
- hard_rules.md, common_pitfalls.md, design_patterns.md
- constraint_examples.md, pattern_examples.md, constraint_rules_summary.md
- constraint_query_table.md, pattern_query_table.md

格式要求：纯 markdown，内容从项目真实代码提取，不可杜撰。
...
这会将 content 文件填入模板，生成最终的 .claude/skills/architect-skill/ 目录（4个文件）。
...
将分析结果写入 bootstrap-output/content/debug/ 目录：
- focus_areas.md, examples.md, project_checks.md
- extra_rules.md, extra_pitfalls.md

格式要求：纯 markdown，所有代码示例从项目真实代码提取。
...
将分析结果写入 bootstrap-output/content/profiler/ 目录：
- performance_budgets.md, examples.md, project_optimizations.md
- extra_rules.md, extra_pitfalls.md

格式要求：纯 markdown，优化建议必须有量化收益描述。
...
**前提条件**：
- 项目已完成全量初始化（`.claude/templates/` 目录存在）

**流程**：

1. **确认模块信息**（主 agent）
   - 向用户确认：模块 ID、显示名、颜色（可选）
   - 确认模块的主要代码目录
   - 生成 manifest（仅含该模块）

2. **启动独立 agent 闭环生成 skill**

   主 agent 组装 prompt（替换占位符），启动独立 agent 完成**分析 + skill 生成**。注意：此阶段只生成 skill，不生成 agent。
   ...
   将分析结果写入 bootstrap-output/content/modules/{MODULE_ID}/ 目录：
   - architecture.md, core_classes.md, relationships.md
   - examples.md, hard_rules.md, pitfalls.md

   格式要求：纯 markdown，所有内容从项目真实代码提取。

   == 第2步：生成 skill ==

   分析完成后，执行：
     python {PROJECT_ROOT}/.castflow/bootstrap.py --skill {MODULE_ID}

   生成：.claude/skills/programmer-{MODULE_ID}-skill/ （4个文件）
...
2. 更新核心文件（SKILL_RULE, GLOBAL_SKILL_MEMORY, code-pipeline-skill, skill-creator）
